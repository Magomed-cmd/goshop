...
type UserService interface {
	Register(ctx context.Context, email, password string, name, phone *string) (*entities.User, string, error)
	Login(ctx context.Context, email, password string) (*entities.User, string, error)
	GetUserProfile(ctx context.Context, userID int64) (*entities.User, error)
	UpdateProfile(ctx context.Context, userID int64, name, phone *string) error
